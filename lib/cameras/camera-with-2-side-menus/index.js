...
class CameraWith2SideMenus extends Camera {
  ...
  _wrappedMenuController(sceneAsChild, active) {
    ...
    const overlay = (active)? this._renderOverlay() : null;
    const animationStyleFn = (active)? animationStyle(this._value) : {};
    const opacityForReplace = {opacity: !active && this._isOpen? 0 : 1};

    return (
      <Animated.View
        key={scene.key}
        style={[opacityForReplace, animationStyleFn]}
        {...panHandlers}>
        {scene}
        {overlay}
      </Animated.View>
    ...
    return (
      <View style={styles.menuWrapper}>
        <Animated.View style={[styles.view, { opacity: this._leftMenuOpacity }]}>
          <LeftMenu key="rightMenu" ref="rightMenuRef" closeMenu={() => { this.closeMenu(); }}/>
        </Animated.View>
        <Animated.View style={[styles.view, { opacity: this._rightMenuOpacity }]}>
          <RightMenu key="rightMenu" ref="rightMenuRef" closeMenu={() => { this.closeMenu(); }}/>
        </Animated.View>
      </View>
    );
  }
