...
class CameraWith2SideMenus extends Camera {
  ...
  _showMenu(side, menuOffset) {
    animationFunction(this._value, menuOffset).start(() => {
      this._prevValue = this._value.__getValue();
      this._isOpen = true;
    });
  }
  ...
  renderBack(camera) {
    ...
    const leftMenu = (
      <Menu
        ...
        ref="leftMenuRef"
        component={LeftMenu}
        width={Math.abs(openLeftMenuOffset)}
        props={{
    ...
    const rightMenu = (
      <Menu
        ...
        ref="rightMenuRef"
        component={RightMenu}
        width={Math.abs(openRightMenuOffset)}
        props={{
          closeMenu: this.closeMenu.bind(this)
        }}/>
    );
    ...
    if (activeSide === 'left') {
      children = [rightMenu, camera, leftMenu];
    } else if (activeSide === 'right') {
      children = [leftMenu, camera, rightMenu];
    } else {
      children = [leftMenu, rightMenu, camera];
    }
...
class Menu extends Component {
  ...
  render() {
    ...
    const { component: Component, width } = this.props;

    return (
      <Animated.View style={[styles.menu, { width }]}>
