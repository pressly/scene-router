...
class CameraWith2SideMenus extends Camera {
  ...
  _wrappedMenuController(sceneAsChild, active) {
  ...
  renderBack() {
    ...
    return (
      <View style={[
              styles.menuWrapper,
              this._isOpen ? { transform: [{'translate':[0,0,1]}] } : null
            ]}>
      <Menu
          key="leftMenu"
          ref="leftMenuRef"
          component={LeftMenu}
          side="left"
          openOffset={openLeftMenuOffset}
          isOpen={this._leftMenuIsOpen}
          props={{
        ...
        <Menu
          ...
          component={RightMenu}
          side="right"
          openOffset={openRightMenuOffset}
          isOpen={this._rightMenuIsOpen}
          props={{
            closeMenu: this.closeMenu.bind(this)
          }}/>
      </View>
  ...
  renderScenes(scenes) {
    ...
    return React.Children.map(scenes, (scene, index) => {
      return this._wrappedMenuController(scene, index == lastIndex);
    });
  }
...
class Menu extends Component {
  ...
  render() {
    const { props, offsetAnim} = this.state;
    const { component: Component, openOffset, side } = this.props;

    return (
      <Animated.View style={[
                             styles.menu,
                             { [side]: offsetAnim, width: openOffset }
                           ]}>
