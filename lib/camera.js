...
class Camera extends Component {
  ...
  _makeInternalProps(side, position, startTransition, withAnimation, path) {
    return {
      //store the side
      side,
  ...
  _buildSceneObj(id, sceneComponent, props, side, withAnimation, path) {
    const { transitionFn } = this.props;
    ...
    const startTransition = async () => {
      try {
        ...
        transitionFn({ status: 'started' });
        await this._move(side, withAnimation)
        transitionFn({ status: 'ended' });
    ...
    const internalProps = this._makeInternalProps(side, nextScenePosition, startTransition, withAnimation, path);
  ...
  pushScene(sceneComponent, id, props, side, withAnimation = true, reset = false, path) {
...
Camera.propTypes = {
  ...
  transitionFn: React.PropTypes.func
};

Camera.defaultProps = {
  transitionFn: (options = { status: 'started | ended' }) => {}
}
