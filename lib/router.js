          ...
          if (requireReset) {
            this.state.names.forEach((name: string) => {
              if (name !== this.state.firstAreaName) {
                ...
                // remove the ref from map
                this.state.areaRefs.delete(name)
              }
            })

            // clean up the names and ares
            this.state.areas.splice(0, this.state.areas.length - 1)
            this.state.names.splice(0, this.state.names.length - 1)
            
            // this make sure all the tabs are destroyed
            this.setState(this.state)
          }
        })
      } else {
